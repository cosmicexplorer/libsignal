...
use crate::{
    message_encrypt, AsymmetricRole, CiphertextMessageType, Context, Direction, IdentityKey,
    IdentityKeyPair, IdentityKeyStore, KeyPair, KeyType, PreKeySignalMessage, PreKeyStore,
...
mod sealed_sender_v2 {
    ...
    pub const MESSAGE_KEY_LEN: usize = PRIVATE_KEY_LENGTH;
    ...
    pub(super) fn apply_agreement_xor(
        ...
        let their_pub_key = their_key.serialize();
        let agreement_key_input = match direction {
            Direction::Sending => [agreement.as_ref(), &our_pub_key, &their_pub_key],
            Direction::Receiving => [agreement.as_ref(), &their_pub_key, &our_pub_key],
        }
        .concat();
        ...
        UnidentifiedSenderMessage::V2 {
            ...
            if !bool::from(keys.e.public_key.ct_eq(&ephemeral_public)) {
                return Err(SignalProtocolError::InvalidSealedSenderMessage(format!(
                    ...
                    keys.e.public_key, &ephemeral_public, encrypted_message
                )));
            }
