...
impl ServerCertificate {
    pub fn deserialize(data: &[u8]) -> Result<Self> {
        ...
        let signature: [u8; SIGNATURE_LENGTH] =
            (*signature).try_into().map_err(|_: Infallible| {
                SignalProtocolError::BadKeyLength(
                    KeyType::Curve25519,
                    AsymmetricRole::Signature,
                    signature.len(),
    ...
    pub fn validate(&self, trust_root: &PublicKey) -> bool {
        if let Some(ref key_id) = self.key_id().ok() {
            if REVOKED_SERVER_CERTIFICATE_KEY_IDS.contains(key_id) {
                log::error!("received server certificate with revoked ID {:x}", key_id);
                return false;
            }
        }
    ...
    fn deserialize(data: &[u8]) -> Result<Self> {
        ...
        match version {
            ...
            SEALED_SENDER_V2_VERSION => {
                ...
                let ephemeral_public: &[u8; PUBLIC_KEY_LENGTH] =
                    &ephemeral_public.to_vec().try_into().map_err(|e: Vec<u8>| {
                        SignalProtocolError::BadKeyLength(
                            curve::KeyType::Curve25519,
                            AsymmetricRole::Public,
                            e.len(),
                ...
                let ephemeral_public: &[u8; 32] =
                    &(*ephemeral_public).try_into().map_err(|_: Infallible| {
                        SignalProtocolError::BadKeyLength(
                            KeyType::Curve25519,
                            AsymmetricRole::Public,
...
pub async fn sealed_sender_encrypt_from_usmc<R: Rng + CryptoRng>(
    ...
    let static_key_ctext = crypto::aes256_ctr_hmacsha256_encrypt(
        ...
        &eph_keys.cipher_key().try_into().map_err(|_: Infallible| {
            SignalProtocolError::BadKeyLength(
                KeyType::Curve25519,
    ...
    let chain_key: [u8; crypto::AES_256_KEY_SIZE] =
        eph_keys.chain_key().try_into().map_err(|_: Infallible| {
            SignalProtocolError::BadKeyLength(
                KeyType::Curve25519,
                AsymmetricRole::SymmetricKey,
...
mod sealed_sender_v2 {
    ...
    pub(super) fn apply_agreement_xor(
        ...
        let agreement_key_input: Vec<u8> = match direction {
            ...
            Direction::Receiving => [agreement.as_ref(), priv_pub.as_ref(), pub_key.as_ref()],
        }
        .concat();
