...
mod tests {
    use super::*;
    use crate::{PrivateKey, PublicKey, RatchetChainMessageVersion};
    ...
    fn test_chain_key_derivation_v2() -> Result<()> {
        ...
        let chain_key = ChainKey::new(HKDF::initialize(RatchetChainMessageVersion::V2), &seed, 0)?;
    ...
    fn test_chain_key_derivation_v3() -> Result<()> {
        ...
        let chain_key = ChainKey::new(HKDF::initialize(RatchetChainMessageVersion::V3), &seed, 0)?;
        assert_eq!(&seed, chain_key.key());
        assert_eq!(&message_key, chain_key.message_keys()?.cipher_key());
        assert_eq!(&mac_key, chain_key.message_keys()?.mac_key());
        assert_eq!(&next_chain_key, chain_key.next_chain_key()?.key());
    ...
    fn test_root_key_derivation_v2() -> Result<()> {
        ...
        let root_key = RootKey::new(
            HKDF::initialize(RatchetChainMessageVersion::V2),
            &root_key_seed,
        )?;
