...
use crate::{
    Context, KeyPair, MessageVersionType, ProtocolAddress, Result, SenderKeyDistributionMessage,
    SenderKeyMessage, SenderKeyMessageVersion, SenderKeyRecord, SenderKeyStore,
    SignalProtocolError,
};
...
pub async fn create_sender_key_distribution_message<R: Rng + CryptoRng>(
    ...
    if sender_key_record.is_empty()? {
        ...
        sender_key_record.set_sender_key_state(
            SenderKeyMessageVersion::current().into(),
            chain_id,
            iteration,
            &sender_key,
    ...
    SenderKeyDistributionMessage::new(
        state.message_version()?.try_into()?,
        distribution_id,
        state.chain_id()?,
